export const bioParagraphs: string[] = [
  '我是一名 AI 工程师，专注于多智能体系统与大模型应用落地。核心方向涵盖 Agent 架构设计（ReAct Loop / Swarm 协作 / Skills-Agent 解耦）、大模型微调（SFT + GSPO RL / PPO + Step-RL）、检索增强生成（KG 扩展→HyDE→双路召回→RRF→Reranker）和 Harness Engineering（YAML 约束驱动 + 运行时验证 + 输出自动修复）。日常使用 Claude Code 等 AI 编程工具加速开发迭代。擅长在有限算力条件下完成从数据管线到模型训练再到推理部署的完整闭环。',
...
export const brandTags: string[] = ['Multi-Agent', 'Skills-Agent', 'SFT + RL', 'LoRA', 'Hybrid RAG', 'Harness Eng', 'Claude Code', 'Full Stack']
