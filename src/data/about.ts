export const bioParagraphs: string[] = [
  '我是一名 AI 工程师，专注于多智能体系统与大模型应用落地。核心方向涵盖 Agent 架构设计（ReAct Loop / Swarm 协作 / Skills-Agent 解耦）、大模型微调（SFT + GSPO RL / PPO + Step-RL）、检索增强生成（KG 扩展→HyDE→双路召回→RRF→Reranker）和 Harness Engineering（YAML 约束驱动 + 运行时验证 + 输出自动修复）。日常使用 AI 编程工具加速开发迭代。具备模型量化（INT8/FP8）、推理部署（vLLM）、数据蒸馏（DeepSeek-R1/doubao-seed-2.0-pro 教师模型）和分布式训练（A100×8 DeepSpeed ZeRO-3）的完整工程经验。',
...
export const brandTags: string[] = ['Multi-Agent', 'MCP', 'RLHF', 'LoRA', 'Hybrid RAG', 'Harness Eng', 'vLLM', 'Quant/Deploy', 'Distributed Tr']
